...
#define ones(x) ((1ULL << i+1) - 1) /*Get a variable of length x with just '1's*/
...
int main(void)
{
    ...
    {
        UlamWords[i] = (char*)calloc(sizeof(char)*(ones(i)/8+1),1);
        if (i < 2) *UlamWords[i] = j<<i, size=2;
        else
            for (uint64_t word = 0; word <= ones(i); word++)
                if (is_ulam(word, UlamWords, i+1))
                    set(UlamWords[i], word), size++;
        fprintf(fp, "Length %d\n%ld\n", i+1, size);
    }
